...
function CarDetailsEdit() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`http://localhost:8080/api/car/update/${id}`, car, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const handleImageUpload = async () => {
    ...
    try {
      await axios.post(`http://localhost:8080/api/car/upload/car-pic?carId=${id}`, formData, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`
        }
      });
        ...
        <div className="col-lg-12">
          <nav aria-label="breadcrumb">
            <ol className="breadcrumb fanta-breadcrumb">
              ...
              <li className="breadcrumb-item"><Link to="/manager/cars">Cars</Link></li>
